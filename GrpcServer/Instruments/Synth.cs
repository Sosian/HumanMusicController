...
public class Synth
{
    ...
    public void PlaySynth(float value)
    {
        ...
        for (int i = 0; i < mappingArray.GetLength(1); i++)
        {
            ...
            {
                var oscMessage = new OscMessage(new Address($"/note/{synth}"), [(float)mappingArray[1, i - 1]]);
                oscSender.SendMessageAsync(oscMessage);
            }
            else if ((mappingArray[0, i] < value) && (mappingArray[0, i + 1] > value))
            {
                var oscMessage = new OscMessage(new Address($"/note/{synth}"), [(float)mappingArray[1, i]]);
                oscSender.SendMessageAsync(oscMessage);
                break;
            }
        }
